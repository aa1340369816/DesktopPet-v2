...
class ChoiceWindow:
    """极简二选一窗口：垂直排列按钮，实时跟随宠物"""
    def __init__(self, parent, event, on_choose, pet_x, pet_y, pet_w, pet_h, pet=None):
        self.parent = parent
        self.pet = pet
        ...
        def make_choice(choice_idx):
            if self.pet:
                self.pet.unregister_follow_window(self.win)
            self.win.destroy()
        ...
        def update_position():
            if self.win.winfo_exists() and self.pet:
                pet_x = self.pet.x
                pet_y = self.pet.y
        ...
        update_position()
        if self.pet:
            self.pet.register_follow_window(self.win, update_position)
...
class EventScheduler:
    def __init__(self, pet_state, toast_callback, info_callback,
                 float_callback=None, narrative_callback=None, refresh_callback=None, pet=None):
    ...
    def _check_conditions(self, conditions):
        ...
        if "hour_range" in conditions:
            ...
            real_hour = time.localtime().tm_hour
            if not (low <= real_hour < high):
                return False
